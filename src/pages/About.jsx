...
const About = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-10">
            <h1 className="text-4xl font-bold text-center text-red-700 mb-6">About Us</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {owners.map((owner, index) => (
                    <div key={index} className="bg-white shadow-md border border-red-700 rounded-xl p-6 flex flex-col items-center text-center">
                        <img
                            src={owner.image}
                            alt={owner.name}
                            className="w-36 h-36 rounded-full object-cover border-2 border-red-500 mb-4"
                        />
                        ...
                        <p className="text-gray-600"><strong>Email:</strong> <a href={`mailto:${owner.email}`} className="text-red-500">{owner.email}</a></p>
                    </div>
                ))}
            </div>
        </div>
